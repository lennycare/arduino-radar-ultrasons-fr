...
# Système Sonar de Surface avec Arduino

Ce projet combine une carte Arduino, un servomoteur et un capteur de distance à ultrasons pour créer un système de télémétrie à ultrasons. Le capteur est monté sur le servomoteur pour balayer activement un arc de 180 degrés. Les données de distance recueillies sont ensuite traitées pour générer une carte textuelle des obstacles détectés, qui est affichée à la fois sur un écran LCD 16x2 et via le moniteur série.
...
Le titre **« Arduino Surface Sonar System »** a été choisi pour refléter précisément la technologie utilisée ainsi que son environnement d'utilisation :

* **Sonar :** À la base, le principe de fonctionnement du projet est celui du **SONAR** (*Sound Navigation and Ranging*). Tout comme les systèmes maritimes, cet appareil localise les obstacles en émettant des impulsions ultrasonores et en calculant la distance grâce aux échos renvoyés.
* **Surface :** Bien que la technologie s'inspire des applications navales, le terme **« Surface »** a été ajouté pour distinguer clairement ce projet des appareils sous-marins. Il précise que le système est conçu pour fonctionner à l'air libre (sur le sol ou une table) afin de scanner un plan en 2D, et non sous l'eau.